    ...
    <div class="sidebar-menu">
        <ul class="main-menu">
           <!--  @foreach($menus as $menuItem)
            ...
            @if(clean($menuItem['title']) != "Dashboard" )
                @if(clean($menuItem['title']) != "Booking History" )
                    @if(clean($menuItem['title']) != "Wishlist" )
                        @if(clean($menuItem['title']) != "Manage Hotel" )
                            @if(clean($menuItem['title']) != "Manage Car" )
                                @if(clean($menuItem['title']) != "Manage Car" )
                                    @if(clean($menuItem['title']) != "Manage Event" )
                                        @if(clean($menuItem['title']) != "Verifications" )
                                            @if(clean($menuItem['title']) != "My Profile" )
                                                @if(clean($menuItem['title']) != "Change Password" )
                                                    @if(clean($menuItem['title']) != "Admin Dashboard" )
                                                        @continue
                                                    @endif
                                                @endif
                                            @endif
